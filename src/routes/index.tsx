              ...
              {displayUser &&
                (!displayProfile ? (
                  ...
                  <div className="glass flex items-center gap-2 rounded-full px-3 py-1.5">
                    <Flag
                      code={localChampionCode || displayProfile.country_code || "cr"}
                      size={22}
                    />
              ...
              {featuredPrediction ? (
                <div className="flex items-center gap-3">
                  <span className="font-semibold text-white/60">
                    Tu pronóstico:{" "}
                    <span className="font-display text-primary text-sm font-bold bg-white/5 px-2.5 py-1 rounded-xl border border-white/5">
                      {featuredPrediction.home} - {featuredPrediction.away}
                    </span>
                  </span>
                  <Link
                    to="/matches"
      ...
      <section className="mt-5 px-4">
        ...
        >
          ...
          <div className="relative flex-1">
            <div className="text-[10px] font-bold uppercase tracking-widest text-[oklch(0.9_0.18_85)]">
            ...
            {champion ? (
              <div className="mt-0.5 flex items-center gap-1.5 text-xs text-white/60">
                Tu predicción: <Flag code={champion.code} size={16} />{" "}
        ...
        <div className="mb-4 flex items-end justify-between">
          <div>
            ...
            <p className="text-xs text-white/50">
              Guarda tus predicciones antes del pitazo inicial
            </p>
          </div>
          <Link to="/matches" className="text-xs font-semibold text-primary">
            Ver todos →
          </Link>
        </div>
...
function StatCard({
  ...
  return (
    <div className="glass relative overflow-hidden rounded-2xl p-4">
      <div
        className={`mb-2 grid h-9 w-9 place-items-center rounded-xl ${accent === "gold" ? "bg-[oklch(0.85_0.16_85/0.18)] text-[oklch(0.9_0.18_85)]" : "bg-primary/15 text-primary"}`}
      >
        {icon}
      </div>
      <div className="text-[10px] uppercase tracking-widest text-white/50">{label}</div>
      <div
        className={`font-display text-3xl ${accent === "gold" ? "text-gradient-gold" : "text-gradient-neon"}`}
      >
        {value}
      </div>
    </div>
